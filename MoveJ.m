function MoveJ(x_b,y_b,z_b,v_qd)
    ...
    Error=CheckPoint_ABC(A,B,C,2,R);
    if(Error==1) 
        sprintf('Bị lỗi ở quá trình thứ %d',WP)
        setappdata(0, 'Error_old', Error); 
        return;
    else   
        sprintf('Quỹ đạo của lệnh MoveJ ở quá trình thứ %d thỏa',WP)
    end
    ts=0.05;
    gamma=2;   
    %% Khoi tao bien cuc bo va gia tri ban dau cho cac bien cuc bo
    count=0;
    l0=185;    
    l1=87;    %272-185
    l2=200;
    ...
    [theta1_Final,theta2_Final,d_Final]=DHN(x_b,y_b,z_b,theta1_pre);
    ...
    %% Vong lap mo phong
    for i=1:1:((tf/ts)+3)
        phantram=Quintic(t,a0,a1,a2,a3,a4,a5);
        
        theta1  =theta1_zero    +phantram*(theta1_Final-theta1_zero)/100;
        theta2  =theta2_zero    +phantram*(theta2_Final-theta2_zero)/100;
        d       =d_zero         +phantram*(d_Final-d_zero)/100;
        
        [Px,Py,Pz]=DHT(theta1,theta2,d);
...
%         end
        ...
        %% Check dieu kien va dap
            if((t>(30*ts))&&(t<(tf-ts*3)))
                if(theta1_dot>60*pi/180)
